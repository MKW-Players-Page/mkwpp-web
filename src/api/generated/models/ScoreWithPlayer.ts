/* tslint:disable */
/* eslint-disable */
/**
 * Mario Kart Wii Players\' Page API
 * The brains of the Mario Kart Wii Players\' Page.
 *
 * The version of the OpenAPI document: 0.1.0
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from "../runtime";
import type { PlayerBasic } from "./PlayerBasic";
import {
  PlayerBasicFromJSON,
  PlayerBasicFromJSONTyped,
  PlayerBasicToJSON,
} from "./PlayerBasic";
import type { CategoryEnum } from "./CategoryEnum";
import {
  CategoryEnumFromJSON,
  CategoryEnumFromJSONTyped,
  CategoryEnumToJSON,
} from "./CategoryEnum";

/**
 *
 * @export
 * @interface ScoreWithPlayer
 */
export interface ScoreWithPlayer {
  /**
   *
   * @type {number}
   * @memberof ScoreWithPlayer
   */
  readonly id: number;
  /**
   *
   * @type {number}
   * @memberof ScoreWithPlayer
   */
  rank: number;
  /**
   * Finish time in milliseconds (e.g. 69999 for 1:09.999).
   * @type {number}
   * @memberof ScoreWithPlayer
   */
  value: number;
  /**
   *
   * @type {PlayerBasic}
   * @memberof ScoreWithPlayer
   */
  player: PlayerBasic;
  /**
   *
   * @type {number}
   * @memberof ScoreWithPlayer
   */
  track: number;
  /**
   *
   * @type {CategoryEnum}
   * @memberof ScoreWithPlayer
   */
  category: CategoryEnum;
  /**
   * Off for 3lap, on for flap.
   * @type {boolean}
   * @memberof ScoreWithPlayer
   */
  isLap?: boolean;
  /**
   *
   * @type {number}
   * @memberof ScoreWithPlayer
   */
  standard: number;
  /**
   *
   * @type {number}
   * @memberof ScoreWithPlayer
   */
  recordRatio: number;
  /**
   *
   * @type {Date}
   * @memberof ScoreWithPlayer
   */
  date?: Date;
  /**
   *
   * @type {string}
   * @memberof ScoreWithPlayer
   */
  videoLink?: string | null;
  /**
   *
   * @type {string}
   * @memberof ScoreWithPlayer
   */
  ghostLink?: string | null;
}

/**
 * Check if a given object implements the ScoreWithPlayer interface.
 */
export function instanceOfScoreWithPlayer(
  value: object,
): value is ScoreWithPlayer {
  if (!("id" in value) || value["id"] === undefined) return false;
  if (!("rank" in value) || value["rank"] === undefined) return false;
  if (!("value" in value) || value["value"] === undefined) return false;
  if (!("player" in value) || value["player"] === undefined) return false;
  if (!("track" in value) || value["track"] === undefined) return false;
  if (!("category" in value) || value["category"] === undefined) return false;
  if (!("standard" in value) || value["standard"] === undefined) return false;
  if (!("recordRatio" in value) || value["recordRatio"] === undefined)
    return false;
  return true;
}

export function ScoreWithPlayerFromJSON(json: any): ScoreWithPlayer {
  return ScoreWithPlayerFromJSONTyped(json, false);
}

export function ScoreWithPlayerFromJSONTyped(
  json: any,
  ignoreDiscriminator: boolean,
): ScoreWithPlayer {
  if (json == null) {
    return json;
  }
  return {
    id: json["id"],
    rank: json["rank"],
    value: json["value"],
    player: PlayerBasicFromJSON(json["player"]),
    track: json["track"],
    category: CategoryEnumFromJSON(json["category"]),
    isLap: json["is_lap"] == null ? undefined : json["is_lap"],
    standard: json["standard"],
    recordRatio: json["record_ratio"],
    date: json["date"] == null ? undefined : new Date(json["date"]),
    videoLink: json["video_link"] == null ? undefined : json["video_link"],
    ghostLink: json["ghost_link"] == null ? undefined : json["ghost_link"],
  };
}

export function ScoreWithPlayerToJSON(
  value?: Omit<ScoreWithPlayer, "id"> | null,
): any {
  if (value == null) {
    return value;
  }
  return {
    rank: value["rank"],
    value: value["value"],
    player: PlayerBasicToJSON(value["player"]),
    track: value["track"],
    category: CategoryEnumToJSON(value["category"]),
    is_lap: value["isLap"],
    standard: value["standard"],
    record_ratio: value["recordRatio"],
    date:
      value["date"] == null
        ? undefined
        : value["date"].toISOString().substring(0, 10),
    video_link: value["videoLink"],
    ghost_link: value["ghostLink"],
  };
}
